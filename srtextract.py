...
def extract_srt(file_path):
    ...
    print('\nprocessing {}'.format(os.path.basename(file_path)))
    ...
    if os.path.exists(ass_path) and not os.path.exists(srt_path):
...
def extract_ass(file_path, ass_path):
    ...
    try:    
        trackn = int(trackns) - 1
    except:
        print('> subtitles tracks incorrect: {}'.format(trackns))
        return
